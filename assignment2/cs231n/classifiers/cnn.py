...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        #컨볼루션은 여기까지 이후는 아핀, stride는 1이라고 하자
        pad = (filter_size - 1) //2   #입력과 출력이 같아야하므로
        C, H, W = input_dim
        OH = (H + 2 * pad - filter_size) + 1
        OW = (W + 2 * pad - filter_size) + 1
        # 2*2 맥스풀링
        OH = OH/2
        OW = OW/2
        
        after_conv_size = np.int(OH*OW*num_filters)
        
        self.params['W2'] = np.random.randn(after_conv_size,hidden_dim)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        out, cache2 = affine_forward(out, W3, b3)
        scores = out
