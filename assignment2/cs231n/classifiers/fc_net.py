...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
                 ...
                 weight_scale=0.01, dtype=np.float32, seed=None):
        """
    ...
    def loss(self, X, y=None):
        ...
        #어떤 normalization 할까?
        if self.normalization == 'batchnorm':
            out, batch_cache[1] = batchnorm_forward(out, self.params['gamma'+str(1)], self.params['beta' + str(1)], self.bn_params[0])
        elif self.normalization == 'layernorm':
            out, batch_cache[1] = layernorm_forward(out, self.params['gamma'+str(1)], self.params['beta'+str(1)], self.bn_params[0])
        
        #드롭아웃
        if self.use_dropout:
            out, drop_cache[1] = dropout_forward(out, self.dropout_param)
            
        out, rl_cache[1] = relu_forward(out)
        
        #반복문돌며 아핀, (정규화,드롭아웃), 렐루 해주기
        for i in range(2, self.num_layers):
            out, cache[i] = affine_forward(out, self.params['W'+str(i)], self.params['b'+str(i)])
            if self.normalization == 'batchnorm':
                out, batch_cache[i] = batchnorm_forward(out, self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
            elif self.normalization == 'layernorm':
                out, batch_cache[i] = layernorm_forward(out, self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
            #드롭아웃
            if self.use_dropout:
                out, drop_cache[i] = dropout_forward(out, self.dropout_param)
            
            out, rl_cache[i] = relu_forward(out)
        ...
        dout, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(dout, cache[self.num_layers])
        loss += 0.5 * self.reg * np.sum(self.params['W' + str(self.num_layers)]*self.params['W'+str(self.num_layers)])
        grads['W'+str(self.num_layers)] += self.reg* self.params['W'+str(self.num_layers)]

        
        for i in range(self.num_layers - 1, 0, -1):
            dout = relu_backward(dout, rl_cache[i])
            #어떤 정규화를 썻는가
            if self.normalization == 'batchnorm':
                dout, grads['gamma'+str(i)], grads['beta'+str(i)] = batchnorm_backward(dout, batch_cache[i])
            
            elif self.normalization == 'layernorm':
                dout, grads['gamma'+str(i)], grads['beta'+str(i)] = layernorm_backward(dout, batch_cache[i])
